...
void main(int round, monster mob, string page_text) {
    ...
    while (current_round() > 0 && current_round() < 30) {
        ...
        if (!extracted) {
            extracted = true;
            if (try_skill_once(page_text, $skill[Extract])) {
                if (current_round() <= 0) return;
            }
        }
        ...
        if (current_round() == before) {
            ...
            if (stalled_rounds >= 3) {
                abort("LoopTheSeaPearlCCS stalled while fighting " + mob + ".");
            }
        } else {
            stalled_rounds = 0;
        }
    ...
    if (current_round() > 0) {
        abort("LoopTheSeaPearlCCS reached round " + current_round()
            + " while fighting " + mob + ".");
    }
}
